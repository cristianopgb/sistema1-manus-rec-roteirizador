...
const hasNonEmptyText = (value: unknown): boolean => {
  ...
  if (!text) return false
  return !['nat', 'nan', 'null', 'undefined', '-', '—', 'sem agenda', 'sem agendamento'].includes(text)
}

const normalizarTextoOperacional = (value: unknown): string | null => {
  const text = toText(value)
  if (!text) return null
  return hasNonEmptyText(text) ? text : null
}
    ...
    const registrosItens = itensM7.map((item, index) => {
      ...
      return {
        ...
        inicio_entrega: pickFirstText(
          normalizarTextoOperacional(item.inicio_entrega),
          normalizarTextoOperacional(item.hora_inicio_entrega),
          normalizarTextoOperacional(item.hora_inicio_janela),
          normalizarTextoOperacional(item.inicio_janela_entrega),
          normalizarTextoOperacional(item.data_agenda),
        ),
        fim_entrega: pickFirstText(
          normalizarTextoOperacional(item.fim_entrega),
          normalizarTextoOperacional(item.hora_fim_entrega),
          normalizarTextoOperacional(item.hora_fim_janela),
          normalizarTextoOperacional(item.fim_janela_entrega),
          normalizarTextoOperacional(item.hora_agenda),
        ),
        ...(inserirPayloadItem ? {
          payload_apoio_json: {
            ...item,
